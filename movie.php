  ...
  <body style="background-color:powderblue;">
    ...
    <?php
       ...
       $res2 = mysql_query($nameQuery, $cxn);
         ...
         while($row = mysql_fetch_row($res2)){
          ...
          for($i = 0; $i < $num2; $i++){
           ...
           else { echo "<td><center>" . $row[$i] . "</center></td>";}
          }
	 echo "</tr>";
	 }		 
         echo "</table>";
       ...
       $genreQuery = "SELECT DISTINCT * FROM MovieGenre mg WHERE mg.mid=".$id;
       $res2 = mysql_query($genreQuery, $cxn);
         echo "<h4> Genre(s):</h4><br>";
         $num2 = mysql_num_fields($res2);       
         echo "<table border='1', cellpadding=4>";
         while($row = mysql_fetch_row($res2)){
          echo "<tr>";
	 ...
	 echo "</tr>";
       ...
       $ratingsQuery="SELECT DISTINCT * FROM MovieRating mr WHERE mr.mid=".$id;
       $res2 = mysql_query($ratingsQuery, $cxn);
         $num2 = mysql_num_fields($res2);
         echo "<h4> Critic Ratings: </h4><br>";       
   echo "<table border='1', cellpadding=4>";
         echo "<tr>";
         for($i=1; $i < $num2; $i++){
	     if (mysql_field_name($res2, $i) == "rot"){
	     echo "<th>"; echo "Rotten Tomatoes"; echo "</th>";
	     } else {
	     echo "<th>"; echo strtoupper(mysql_field_name($res2, $i)); echo "</th>";
         }}
         echo "</tr>";
         while($row = mysql_fetch_row($res2)){
          echo "<tr>";
	 ...
	 echo "</tr>";
      ...
      $directorsQuery = "SELECT DISTINCT * FROM Director m JOIN MovieDirector md WHERE m.id=md.did AND md.mid=".$id;
	...
	echo "</tr>";
      ...
      $reviewsQuery = "SELECT DISTINCT *, avg(r.rating) as 'Average Rating' FROM Review r WHERE r.mid=".$id;
       $res2 = mysql_query($reviewsQuery, $cxn);
       if (is_null(mysql_fetch_row($res2)[0])){
         echo "<h4> No Listed User Reviews.<h4><br>";
       } else {
	 $res2 = mysql_query($reviewsQuery, $cxn);
         echo "<h4> User Reviews:<h4><br>";
         $num2 = mysql_num_fields($res2);       
